...
function processJSONBody(req, res, controller, methodName) {
    ...
    req.on('data', chunk => {
        ...
        try {
            ...
            if (req.headers['content-type'] === "application/json" || req.headers['content-type'] === "application/x-www-form-urlencoded; charset=UTF-8") {
...
exports.dispatch_API_EndPoint = function(req, res){
    ...
    if (controllerName != undefined) {
        ...
        try{
            ...
            // instanciate the controller       
            let needAuthorized = controllerName === "BookmarksController" && (req.method === 'POST' || req.method ===  'PUT' || req.method ===  'DELETE')
            let controller =  new Controller(req, res, needAuthorized);
            //console.log(controller.requestAuthorized());
            if (!controller.requestAuthorized()){
                console.log('unauthorized access!');
                response.unAuthorized();
                // request consumed
